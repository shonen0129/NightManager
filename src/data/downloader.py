...
- Orchestrating cache reads/writes via data.cache
...
def _try_apply_nav_patch(
    ...
    nav_csv_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "ETF_1629.csv"
    )
...
def _update_stale_cache_missing_only(
    ...
    if us_close_new.empty or jp_close_new.empty or jp_open_new.empty:
        raise ValueError(
            "Incremental download returned empty market data "
            "(us_close/jp_close/jp_open)"
        )

    # Normalize indices and merge (old values take priority via combine_first)
    df_us_close = _to_daily_index(us_close_old).combine_first(
        _to_daily_index(us_close_new)
    ).sort_index()
    df_jp_close = _to_daily_index(jp_close_old).combine_first(
        _to_daily_index(jp_close_new)
    ).sort_index()
    df_jp_open = _to_daily_index(jp_open_old).combine_first(
        _to_daily_index(jp_open_new)
    ).sort_index()
...
def download_data(
    ...
    if has_stale_cache:
        ...
        try:
            ...
            if not _has_sufficient_topix_history(updated.get("jp_close"), beta_window):
                raise ValueError(
                    "TOPIX proxy history is insufficient after incremental update"
                )
    ...
    try:
        ...
        if us_close.empty or jp_close.empty or jp_open.empty:
            raise ValueError(
                "Downloaded market data is empty (us_close/jp_close/jp_open)"
            )

        df_jp_close, df_jp_open = _try_apply_nav_patch(
            jp_close.copy(), jp_open.copy()
        )
        ...
        if has_stale_cache and os.path.exists(pkl_path):
            logger.warning(
                "Download failed. Falling back to stale cache: %s", pkl_path
            )
            return pd.read_pickle(pkl_path)
        raise
...
def update_intraday_cache(tickers: list[str] = JP_TICKERS) -> None:
    ...
    """
    from data.cache import load_intraday_cache, save_intraday_cache
    ...
    for interval, period in [("1m", "7d"), ("5m", "60d")]:
        ...
        try:
            ...
            logger.info(
                ...
                interval, len(combined), start_dt, end_dt
            )
        except Exception as e:
