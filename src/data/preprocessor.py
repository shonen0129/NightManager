...
``data.downloader.download_data()`` into the ``df_exec`` DataFrame used by
...
def preprocess_data(data: dict, beta_window: int = 60) -> pd.DataFrame:
    ...
    if "XLC" in ret_us_cc.columns and ret_us_cc["XLC"].isna().any():
        ...
        ret_us_cc["XLC"] = ret_us_cc["XLC"].fillna(
            (ret_us_cc["XLK"] + ret_us_cc["XLY"]) / 2
        )
    ...
    if "MTUM" in ret_us_cc.columns and "IUSG" in ret_us_cc.columns and ret_us_cc["MTUM"].isna().any():
    ...
    if "USMV" in ret_us_cc.columns and ret_us_cc["USMV"].isna().any():
        ...
        ret_us_cc["USMV"] = ret_us_cc["USMV"].fillna(
            (ret_us_cc["XLP"] + ret_us_cc["XLV"]) / 2
        )
