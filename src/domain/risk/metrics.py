...
from domain.models.types import RiskConfig, RiskReport, VarEsResult
...
def evaluate_risk_checks(
    ...
    if float(max_capital) > 0:
        allocated_net_ratio = abs(total_buy_allocated - total_sell_allocated) / float(
            max_capital
        )
        allocated_gross_ratio = (total_buy_allocated + total_sell_allocated) / float(
            max_capital
        )
    else:
    ...
    if var_es.available:
        ...
        if var_loss >= config.var_stop:
            ...
            warning_breaches.append(
                f"VaR99={var_loss:.4%} >= warning {config.var_warning:.2%}"
            )
        ...
        if es_loss >= config.es_stop:
            ...
            warning_breaches.append(
                f"ES99={es_loss:.4%} >= warning {config.es_warning:.2%}"
            )
    else:
        warning_breaches.append(
            f"VaR/ES skipped due to insufficient history "
            f"({var_es.samples}/{var_es.window})"
        )
