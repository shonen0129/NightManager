...
from domain.models.types import CapitalAllocation
...
def allocate_capital(
    ...
    sell_abs_sum = (
        float(np.sum(-weights[sell_indices])) if len(sell_indices) > 0 else 0.0
    )
    ...
    def _allocate_side(
        indices: np.ndarray, abs_weights: np.ndarray, side_budget: float
    ):
    ...
    if max_net_exposure is not None and float(max_capital) > 0:
        ...
        def _side_totals() -> tuple[float, float]:
            buy_total = (
                float(np.sum(allocated[buy_indices])) if len(buy_indices) else 0.0
            )
            sell_total = (
                float(np.sum(allocated[sell_indices])) if len(sell_indices) else 0.0
            )
