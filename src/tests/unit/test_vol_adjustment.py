...
def test_compute_signal_vol_adjustment():
    ...
    v0_static = signals.build_v3_static(n_us, n_jp, include_v4=True)
    base_vectors = signals.build_base_vectors(n_us, n_jp)
    ...
    sig_raw = signals.compute_signal(
        ...
        n_us,
        corr_window,
        signals.compute_baseline_correlation(all_returns, date_index, ewma_half_life),
        v0_static,
        v1,
        v2,
        k,
    ...
    jp_returns_20 = all_returns[test_idx - 19 : test_idx + 1, n_us:]
...
def test_vol_adjusted_signal_no_leakage():
    ...
    test_idx = 75
    c_full = signals.compute_baseline_correlation(all_returns, date_index, ewma_half_life)
    ...
    # 2. Corrupt returns starting from test_idx + 1
