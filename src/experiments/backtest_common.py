...
def simulate_overnight_holding(
    ...
    for i, date in enumerate(dates):
        ...
        if callable(alpha):
            sig_row = signals_df.loc[date].values if signals_df is not None else None
            alpha_mask = np.asarray(alpha(i, date, w_t, r_target, sig_row), dtype=float)
        else:
        ...
        overnight = 0.0
        if np.any(alpha_mask > 0) and i < len(dates) - 1:
            next_date = dates[i + 1]
