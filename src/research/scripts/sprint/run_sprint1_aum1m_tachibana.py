...
def main():
    ...
    artifact_dir = config.get("artifact_dir", "var/artifacts/sprint1_aum1m_tachibana")
    ...
    targets_pivot["close_to_close_return"].reindex(valid_dates)
    ...
    df_exec["topix_oc_return"].reindex(valid_dates)
    ...
    config.get("sell_interest_rate_annual", 0.00)
    ...
    (1.0 + r_topix_cc).cumprod() - 1.0
    (1.0 + strat_net).cumprod() - 1.0
    ...
    with open(report_path, "w") as f:
        f.write(rf"""# Sprint 1 — AUM 100万円・立花証券信用取引コスト対応 定量検証レポート
