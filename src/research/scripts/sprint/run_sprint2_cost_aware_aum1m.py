...
def main():
    ...
    config.get("impact_eta_list", [0.0, 0.02, 0.05, 0.10])
    config.get("min_adv_jpy_list", [0, 500000, 1000000, 3000000])
    ...
    pd.DataFrame(short_unavail_rows)
    pd.DataFrame(gross_comp_rows)
    ...
    with open(report_path, "w") as f:
        f.write(rf"""# Sprint 2 — AUM 100万円・コスト控除後最適化モデル定量検証レポート
