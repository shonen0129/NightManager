#!/usr/bin/env python
"""Phase 1 experiment entry point for ML order decision overlay.

Usage:
    python3 scripts/experiments/experiment_ml_order_decision.py
"""
...
from research.experiments.ml_order_decision.phase1 import run_phase1_experiment
...
def main():
    parser = argparse.ArgumentParser(description="Phase 1 ML order decision overlay")
    parser.add_argument(
        "--gap-input-dir",
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "reports" / "ml_order_decision" / "phase1_results"),
    ...
    parser.add_argument(
        "--per-ticker-interactions",
        action="store_true",
        help="Use one-hot ticker x score / gap / score_x_gap interaction features",
    )
    parser.add_argument(
        "--n-jobs",
        type=int,
    ...
    logger.info("Running Phase 1 experiment...")
