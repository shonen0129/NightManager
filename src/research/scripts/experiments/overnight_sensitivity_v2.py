...
def main():
    ...
    for name, alpha_long, alpha_short in scenarios:
        ...
        record_backtest_experiment(
            name=f"{Path(__file__).stem}_{name}",
            hypothesis=f"Overnight holding fraction sensitivity (long={alpha_long}, short={alpha_short}).",
            app_config=base_app_config,
            results=results,
            extra_metrics={"alpha_long": alpha_long, "alpha_short": alpha_short, "scenario": name},
