#!/usr/bin/env python3
"""Validate expanded subsector basket quality (1620-stock, value-weighted).

Outputs:
  var/research/subsector/quality_summary_expanded_vw.json
  reports/subsector_refinement/phase0_data/quality_report_expanded_vw.md
"""
...
def main() -> int:
    panel_path = ROOT / "var" / "research" / "subsector" / "panel_subsector_oc_expanded_vw.parquet"
    ...
    _coverage_list = agg.get("coverage", [])
    if isinstance(_coverage_list, list):
        coverage = {c["sector_etf"]: c for c in _coverage_list}
    else:
        coverage = _coverage_list
    ...
    summary_path = ROOT / "var" / "research" / "subsector" / "quality_summary_expanded_vw.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    report_path = out_dir / "quality_report_expanded_vw.md"
    lines = [
        ...
        "",
        "## Phase 0 Gates",
        f"- (1) Unmapped tickers in expanded mapping: {agg['meta'].get('unmapped_tickers', 'n/a')} (need 0) — see mapping meta",
