...
def _run_one(args: tuple) -> dict:
    ...
    record_backtest_experiment(
        ...
        hypothesis=f"PIT IR history length sensitivity (max_pit={max_pit}).",
        app_config=app_config,
        results=results,
        extra_metrics={"max_pit_history": max_pit},
        decision=Decision.PENDING,
    )
    return _metrics(results, max_pit, label)
...
def main():
    ...
    parser.add_argument("--results-dir", default=f"var/results/pit_history_sensitivity_{datetime.now().strftime('%Y%m%d')}")
