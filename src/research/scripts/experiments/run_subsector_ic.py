#!/usr/bin/env python3
"""Run subsector BLPX signal and evaluate aggregated IC against 17 TOPIX ETFs.

The model is run in the 79-dimensional subsector return space.  Predicted
subsector signals are aggregated back to the 17 TOPIX-17 ETF space using the
market-cap-weighted A matrix.  The daily rank IC is then computed against the
realized open-to-close returns of the 17 ETFs.

Outputs:
  reports/subsector_refinement/phase2_blpx/subsector_ic_report.md
"""
...
# Canonical 17 TOPIX-17 ETF tickers (before any monkey-patching)
...
def _set_jp_universe(names: list[str], is_subsector: bool = False) -> None:
    ...
    if not is_subsector:
        # Restore original tickers.py state (includes SENSITIVITY_LABELS)
    ...
    if is_subsector:
        # Extend sensitivity labels: each subsector inherits its dominant ETF's labels
        ...
        # Remove stale 17-ETF JP sensitivity entries only when in subsector mode
        for tk in _ORIGINAL_JP_TICKERS:
            _tickers.SENSITIVITY_LABELS.pop(tk, None)
    # Rebind module-level imports so that functions see the new list
...
# Snapshot the original sector mapping so we can restore it for the 17-dim baseline model
...
# Original US -> 17 ETF mapping from the prior builder
...
def main() -> int:
    ...
    # Build baseline 17-dim df_exec for comparison (use canonical JP universe)
    ...
    # Switch to 79-dim subsector universe for subsector BLPX
    ...
    cfg = BLPXConfig(
        ...
        param_set="subsector_experiment",
    ...
    # Use the residual-BLPX signal DataFrame (columns = 79 subsectors)
    ...
    # Aggregate to 17 ETFs
    signal_17_from_sub = signal_sub @ A.T  # (T, 17)
    signal_17_df = pd.DataFrame(signal_17_from_sub, index=df_exec_sub.index, columns=ETF_ROWS)

    # Align actual 17 ETF oc returns
    ...
    # Also compute direct 17-dim BLPX as baseline (restore 17-dim universe)
    _set_jp_universe(_ORIGINAL_JP_TICKERS)
    _prior_builder._SECTOR_MAPPING_STRUCTURE = _ORIGINAL_SECTOR_MAP_FOR_RESTORE
    ProductionBLPXModel._SECTOR_MAPPING_STRUCTURE = _ORIGINAL_SECTOR_MAP_FOR_RESTORE
    ...
    with open(out_dir / "subsector_ic_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    report_lines = [
        "# Phase 2: Subsector BLPX IC Evaluation",
