...
def parse_args():
    ...
    p.add_argument("--output-dir", default="var/results/v2_backtest_live_aligned")
    return p.parse_args()
...
def load_actual_wallet_close(trade_date: str) -> float | None:
    dirs = list(ROOT.glob(f"var/results/{trade_date.replace('-','')}_*_production_close_positions"))
    if not dirs:
        return None
...
def main():
    ...
    record_simple_experiment(
        ...
        hypothesis="Live-aligned V2 backtest using per-day 'latest' gap distributions and overlay.",
        parameters=app_config.model_dump(),
        metrics={
            "n_days": int(len(valid)),
            "final_equity": float(equity),
            "avg_daily_net_ret": float(valid["bt_net_return"].mean()) if len(valid) > 0 else 0.0,
            "avg_weight_rmse": float(valid["weight_rmse"].mean()) if len(valid) > 0 else 0.0,
            "avg_sign_agreement": float(valid["sign_agreement"].mean()) if len(valid) > 0 else 0.0,
        },
