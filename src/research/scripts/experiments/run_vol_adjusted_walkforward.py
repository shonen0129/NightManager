...
def main() -> int:
    ...
    record_backtest_experiment(
        name=f"{Path(__file__).stem}_false",
        hypothesis="Walk-forward: vol_adjusted_target=false vs baseline.",
        app_config=app_config,
        results=res_false,
        decision=Decision.PENDING,
    )
    record_backtest_experiment(
        name=f"{Path(__file__).stem}_true",
        hypothesis="Walk-forward: vol_adjusted_target=true vs baseline.",
        app_config=app_config,
        results=res_true,
        decision=Decision.PENDING,
    )
