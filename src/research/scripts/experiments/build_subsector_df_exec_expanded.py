#!/usr/bin/env python3
"""Build a 79-dimensional df_exec using expanded value-weighted subsector returns.

Outputs:
  var/research/subsector/df_exec_subsector_expanded_vw.parquet
"""
...
def main() -> int:
    ...
    cc = pd.read_parquet(ROOT / "var" / "research" / "subsector" / "panel_subsector_cc_expanded_vw.parquet")
    ...
    out_path = ROOT / "var" / "research" / "subsector" / "df_exec_subsector_expanded_vw.parquet"
    df_exec.to_parquet(out_path)
    logger.info("Saved df_exec_subsector_expanded_vw: %s", df_exec.shape)
    return 0
