...
This module wraps the existing ``kabu_client.KabuClient`` behind the
...
The underlying KabuClient is kept intact; this is a **thin adapter**,
not a rewrite.
"""
...
from broker.base import BrokerClient, BrokerConfig, Position, WalletInfo
...
from kabu_client import KabuClient, KabuConfig, KabuApiError, issue_api_token
...
class KabuBrokerClient(BrokerClient):
    ...
    Wraps ``kabu_client.KabuClient`` and translates between the
    ...
    def __init__(self, config: BrokerConfig) -> None:
        ...
        self._kabu_config: Optional[KabuConfig] = None
        self._client: Optional[KabuClient] = None
    ...
    def _connect(self, config: BrokerConfig) -> None:
        ...
        if not api_token:
            api_password = config.api_password or os.environ.get(
                "KABU_API_PASSWORD", ""
            )
            if api_password:
                logger.info(
                    "[KabuBroker] No API token provided. Issuing via /token..."
                )
    ...
    def get_positions(self, **filters: Any) -> list[Position]:
        ...
        for pos in raw:
            ...
            # If it's a bare code like "1617", add ".T"
    ...
    def submit_order(
        ...
        *,
        ...
        # Convert yfinance ticker to kabu code
        clean_ticker = order.ticker.replace(".T", "")

        # Map OrderType enum to kabu order_type string
        ot_map = {
            OrderType.MARKET: "MO",
            OrderType.LIMIT: "LO",
            OrderType.CLOSE: "CLO",
    ...
    def submit_orders_batch(
        ...
        *,
        ...
        The underlying KabuClient.place_orders_batch has market-neutrality
        rollback logic that we want to preserve.
        """
        if not orders:
            return []

        # Convert OrderRequest list to dict list for KabuClient
        order_dicts = []
        for o in orders:
