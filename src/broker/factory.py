...
Usage::
    ...
    from broker import create_broker, BrokerConfig
...
Supports:
    ...
    - (future) ``"sbi"`` → SBIBrokerClient
    - (future) ``"rakuten"`` → RakutenBrokerClient
"""
...
def create_broker(config: BrokerConfig) -> BrokerClient:
    ...
    # Future providers
    # elif provider == "sbi":
    #     from broker.sbi.client import SBIBrokerClient
    #     return SBIBrokerClient(config)

    else:
        raise ValueError(
            f"Unknown broker provider: {config.provider!r}. "
            f"Supported: 'kabu', 'dry_run'"
        )
...
def create_broker_from_args(
    *,
    ...
    """Convenience factory matching production.py's current _build_api_client pattern.

    This function bridges the gap during migration: production.py can call
    this instead of building KabuClient directly.
