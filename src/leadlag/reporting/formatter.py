...
def print_text_orders(decision_df: pd.DataFrame) -> None:
    ...
    buy_orders = decision_df[(decision_df["action"] == "BUY") & (decision_df["quantity"] > 0)]
    sell_orders = decision_df[(decision_df["action"] == "SELL") & (decision_df["quantity"] > 0)]
    ...
    if len(buy_orders) == 0:
        ...
        for _, row in buy_orders.iterrows():
            print(
                ...
                f"| 数量: {int(row['quantity']):>4}株 | 概算: {int(row['etf_amount']):>8,} 円"
            )
...
def print_risk_report(report: dict) -> None:
    ...
    if var_es["available"]:
        logger.info(f"VaR/ES(99%%,250d): VaR={var_es['var_loss']:.4%}, ES={var_es['es_loss']:.4%}")
    else:
        logger.info(
            f"VaR/ES(99%%,250d): skipped (history={var_es['samples']}, required={var_es['window']})"
        )
