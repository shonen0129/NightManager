...
def main() -> None:
    ...
    for (sel_k, opt_k), w_matrix in daily_weights.items():
        ...
        for ir_val in pred_irs:
            if ir_val <= tertiles[0]:
                bin_labels.append("Low")
            elif ir_val <= tertiles[1]:
    ...
    cand_base = "baseline_baseline_style_Fixed"
    cand_ref = "mu_over_sigma_baseline_style_RuleD"
    ...
    for cid, name in [
        (cand_base, "Baseline (Fixed)"),
        (cand_best_fixed, "Best Covariance (Fixed)"),
    ...
    df_perf_all[(df_perf_all["selection_method"] == "mu_over_sigma") & (df_perf_all["optimizer_method"] == "baseline_style") & (df_perf_all["gross_rule"] == "Fixed")].iloc[0]
