...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Residual-BLPX Risk-Adjusted Ranking Validation")
...
def main():
    ...
    if not step3_panel_file.exists():
        logger.error("Step 3/3.5 panel file missing. Cannot retrieve multipliers.")
        sys.exit(1)
    ...
    for (rm, wm), w_matrix in daily_portfolio_weights.items():
        ...
        (r_rec["max_drawdown"] - base_net_ret.min()) # difference in drawdowns
    ...
    next(r for r in perf_records if r["ranking_method"] == "baseline" and r["weighting_method"] == "baseline_style" and r["gross_rule"] == "RuleA")
    next(r for r in perf_records if r["ranking_method"] == "baseline" and r["weighting_method"] == "baseline_style" and r["gross_rule"] == "RuleD")
