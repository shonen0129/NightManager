...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Residual-BLPX Risk-Adjusted Ranking Consistency Audit")
...
def main():
    ...
    logger.info("Loading Step 2 gap adjusted distribution long panel...")
    ...
    if not step3_panel_file.exists():
        logger.error("Step 3/3.5 panel file missing.")
        sys.exit(1)
    ...
    if vol_panel_path.exists():
        logger.info("Loading vol state panel...")
        ...
        logger.warning("Vol state panel not found.")
        df_vol = pd.DataFrame()
    ...
    if len(common_dates) == 0:
        logger.error("No aligned trade dates found.")
        sys.exit(1)
    ...
    df_mults.set_index("trade_date").loc[common_dates]
    ...
    for rm, wm, rule in rescore_cands:
        ...
        if cvar95_diff >= 15.0:
            s_cvar95 = 5.0
        elif cvar95_diff >= 5.0:
            s_cvar95 = 4.0
        elif cvar95_diff >= 0.0:
            s_cvar95 = 3.0
        else:
            s_cvar95 = 1.5
        ...
        if cvar99_diff >= 30.0:
            s_cvar99 = 5.0
        elif cvar99_diff >= 10.0:
            s_cvar99 = 4.0
        elif cvar99_diff >= 0.0:
            s_cvar99 = 3.0
        else:
            s_cvar99 = 1.5
        ...
        if overlap_base >= 8.5:
            s_turnover = 5.0
        elif overlap_base >= 7.0:
            s_turnover = 4.0
        else:
            s_turnover = 3.0

        # 10. Simplicity
        if wm == "baseline_style":
            s_simplicity = 5.0
        elif wm == "equal":
            s_simplicity = 4.0
        else:
        ...
        # 11. Interpretability
        if rm in ["baseline", "mu_over_sigma"]:
            s_interpret = 5.0
        elif rm == "winsor_mu_over_sigma":
            s_interpret = 4.0
        else:
            s_interpret = 3.0
    ...
    df_recalc[(df_recalc["ranking_method"] == "baseline") & (df_recalc["gross_rule"] == "Fixed")].iloc[0]
    ...
    df_recalc[(df_recalc["ranking_method"] == "mu_over_sigma") & (df_recalc["weighting_method"] == "baseline_style") & (df_recalc["gross_rule"] == "RuleA")].iloc[0]
    df_recalc[(df_recalc["ranking_method"] == "winsor_mu_over_sigma") & (df_recalc["weighting_method"] == "baseline_style") & (df_recalc["gross_rule"] == "Fixed")].iloc[0]
    df_recalc[(df_recalc["ranking_method"] == "winsor_mu_over_sigma") & (df_recalc["weighting_method"] == "baseline_style") & (df_recalc["gross_rule"] == "RuleD")].iloc[0]
