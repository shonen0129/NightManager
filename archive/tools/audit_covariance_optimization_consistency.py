...
def main() -> None:
    ...
    cov_val_dir = Path(getattr(args, "cov-opt-dir", None) if hasattr(args, "cov-opt-dir") else getattr(args, "cov_opt_dir", None))
    Path(args.ranking_audit_dir)
    Path(args.ranking_validation_dir)
    ...
    cand_primary_id = getattr(args, "primary_step45-candidate", "").replace(":", "_") if hasattr(args, "primary_step45-candidate") else getattr(args, "primary_step45_candidate", "").replace(":", "_")
    cand_cov_id = getattr(args, "covariance-candidate", "").replace(":", "_") if hasattr(args, "covariance-candidate") else getattr(args, "covariance_candidate", "").replace(":", "_")
    cand_opp_id = getattr(args, "opportunity-candidate", "").replace(":", "_") if hasattr(args, "opportunity-candidate") else getattr(args, "opportunity_candidate", "").replace(":", "_")
    ...
    df_state_orig = df_state_rob_orig.copy()
    ...
    with open(out_dir / "report.md", "w") as f:
        ...
        f.write("- **Corrected Covariance Candidate Classification**: `secondary experimental shadow candidate`\n")
