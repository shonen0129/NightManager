...
2. **要求の起点確認**: リファクタリング対象が Issue / PRD / ユーザー要求 / `AGENTS.md` で示された問題として正しく定義されているか確認。定義がなければ簡易仕様を書く
3. **影響分析**: 変更が及ぶファイル・関数を列挙。不変条件（リーク防止・ベースライン分離・市場中立・ティッカー定義）への影響を確認
4. **テスト確認**: リファクタリング前にテストが通ることを確認:
   - 並列（要 `pytest-xdist`）: `bash scripts/run_tests_parallel.sh`（約8分）
   - 直列（フォールバック）: `python3 -m pytest tests/ -v --timeout=300`（約22分）
5. **TDD シームで実装**: 既存のテスト継ぎ目（seams）を優先する。カバレッジ不足の場合は先にテストを追加してからリファクタリングする。1関数の責務が1つになるよう分割
6. **振る舞い検証**: リファクタリング前後で出力（シグナル・ポートフォリオ・net Sharpe）が一致することを確認（検証コードは下記参照）
7. **lint 実行**: `python3 -m ruff check src/leadlag/ --select E,F,W --ignore E501` でエラーが出ないこと
