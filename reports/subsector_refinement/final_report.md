...
| (3) | バスケット vs ETF 相関 ρ ≥ 0.7 | min=0.27, median=0.51, 74/79 未満 | **FAIL** |
...
### 原因分析
1. **相関ゲート未達**: 74/79 サブセクターの自己バスケット vs ETF 相関が 0.7 未満。サブセクターバスケットが対応 ETF の動きを十分捉えていない。
2. **ノイズ増大**: 79 次元への拡大により、サンプル共分散行列の推定誤差が増大。BLPX の正則化・構造事前分布が過学習/打ち消しを生じた可能性。
3. **集約損失**: A 行列は市場重心を正しく捉えるが、サブセクター信号のノイズも同時に集約される。
...
- 今後同様の細分化を試みる場合、事前にバスケット vs ETF 相関 ≥ 0.7 を達成する定義の再設計が必須。
- 本結果は、本戦略が現在の 17 次元 TOPIX-17 ETF 予測空間を維持すべきであることを支持する。
