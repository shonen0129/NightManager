...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="var/live/pipeline_data/distribution_validation", help="Output directory")
...
def run_self_tests() -> int:
    ...
    np.percentile(history_orig, [33.333333, 66.666667])

    test_ir_leak = test_ir.copy()
    test_ir_leak.iloc[252] = 99.9  # extreme value on day t
    compute_pit_bins(test_ir_leak, "tertile", rolling_window=252)
...
def main():
    ...
    if pred_var_comp_ticker_path.exists():
        pd.read_csv(pred_var_comp_ticker_path)
    ...
    # Ensure date formats (ISO8601 handles both "YYYY-MM-DD" and "YYYY-MM-DD HH:MM:SS")
    df_daily["trade_date"] = pd.to_datetime(df_daily["trade_date"], format="ISO8601").dt.tz_localize(None).dt.normalize()
    df_daily["signal_date"] = pd.to_datetime(df_daily["signal_date"], format="ISO8601").dt.tz_localize(None).dt.normalize()
    df_daily = df_daily.sort_values("trade_date").reset_index(drop=True)

    # Subset by date range
    start_dt = pd.to_datetime(args.start, format="ISO8601")
    end_dt = pd.to_datetime(args.end, format="ISO8601")
    ...
    if args.vol_state_panel is not None:
        ...
        state_df["trade_date"] = pd.to_datetime(state_df["trade_date"], format="ISO8601").dt.tz_localize(None).dt.normalize()
