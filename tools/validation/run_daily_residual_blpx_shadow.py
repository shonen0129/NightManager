...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--production-output-dir", default="var/live/production_residual_blpx", help="Production directory")
    parser.add_argument("--shadow-root", default="var/shadow_runs/residual_blpx", help="Shadow run root folder")
...
def solve_baseline_style(
    ...
    np.concatenate([long_idx, short_idx])
...
def run_self_tests() -> int:
    ...
    solve_mean_variance(mu, Omega_singular, -1.0, np.arange(5), np.arange(5, 10), max_abs_weight=0.1) # Unbounded/infeasible bounds
...
def generate_daily_shadow_portfolio(
    ...
    if np.sum(np.abs(w_base_final)) < 1e-8:
        ...
        logger.warning("Baseline positions are empty. Reconstructing default longs/shorts.")
