...
# Add src/ to path
# File is in tools/validation/, so go up 2 levels to reach project root.
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", default="var/live/pipeline_data/diagnostics_weights", help="Dir with existing results")
    parser.add_argument("--output-dir", default="var/live/pipeline_data/vol_state_diagnostics", help="Output directory")
...
def main():
    ...
    if "raw_blpx_min_pred_var" in model.config or True:
        ...
        if (Path(args.results_dir) / "residual_blpx_diagnostics.csv").exists():
            ...
            diag_df.index = pd.to_datetime(diag_df.index, format="ISO8601")
    ...
    try:
        ...
        if macro_path.exists():
            ...
            if vix_col:
                ...
                vix_data.index = pd.to_datetime(vix_data.index, format="ISO8601").tz_localize(None).normalize()
    ...
    try:
        ...
        if not yf_df.empty:
            for field in ["Open", "High", "Low", "Close", "Volume"]:
                ...
                sub.index = pd.to_datetime(sub.index, format="ISO8601").tz_localize(None).normalize()
