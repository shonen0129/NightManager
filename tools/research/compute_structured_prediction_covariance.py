...
# Add src/ to path
# Script is in tools/research/, so we need to go up 2 levels to reach project root
# tools/research/compute_structured_prediction_covariance.py -> tools/ -> project root
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", default="var/live/pipeline_data/diagnostics_weights", help="Validation outputs directory")
    parser.add_argument("--output-dir", default="var/live/pipeline_data/distribution_diagnostics", help="Output directory")
...
def main():
    ...
    if prev_diag_df is not None and incremental_skipped > 0:
        ...
        df_diag["date"] = pd.to_datetime(df_diag["date"], format="ISO8601")
        df_diag = df_diag.set_index("date").sort_index()
    else:
        df_diag = pd.DataFrame(diag_records).set_index("date")
        df_diag.index = pd.to_datetime(df_diag.index, format="ISO8601")
    ...
    if compare_pred_var:
        if prev_comp_df is not None and incremental_skipped > 0:
            ...
            df_comp["date"] = pd.to_datetime(df_comp["date"], format="ISO8601")
    ...
    if prev_panel_long_df is not None and incremental_skipped > 0:
        ...
        df_panel_long["trade_date"] = pd.to_datetime(df_panel_long["trade_date"], format="ISO8601")
    ...
    if prev_panel_daily_df is not None and incremental_skipped > 0:
        ...
        df_panel_daily["trade_date"] = pd.to_datetime(df_panel_daily["trade_date"], format="ISO8601")
    ...
    if args.vol_state_panel:
        ...
        if state_file.exists():
            ...
            df_state["trade_date"] = pd.to_datetime(df_state["trade_date"], format="ISO8601").dt.strftime("%Y-%m-%d")
    ...
    df_panel_daily["year"] = pd.to_datetime(df_panel_daily["trade_date"], format="ISO8601").dt.year
    ...
    dates_plot = pd.to_datetime(df_diag.index, format="ISO8601")
    plt.figure()
