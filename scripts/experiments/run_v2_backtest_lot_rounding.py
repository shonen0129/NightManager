#!/usr/bin/env python3
"""整数株・ロット制約を適用した V2 バックテスト。

理論値リターン（`compute_jp_target_returns` / `jp_gap`）を使用し、
執行価格のみ 5分足 09:10 の指定モードでロット丸めを行う。
これにより「整数株・ロット制約の影響」を理論値から分離して評価できる。
"""
...
def _find_bar(df_5m: pd.DataFrame, date: pd.Timestamp, ticker: str,
              time_strs: list[str] | None = None) -> pd.Series | None:
...
def main():
    parser = argparse.ArgumentParser(description="V2 backtest with integer lot rounding using theoretical returns")
    parser.add_argument("--entry-mode", default="midpoint", choices=["adverse", "midpoint", "close", "open"],
                        help="09:10 execution price mode used for lot rounding (default: midpoint)")
    ...
    parser.add_argument("--reallocate-unaffordable", action="store_true",
                        help="Redistribute unaffordable/under-allocated notional")
    ...
    # Load theoretical weights and returns
    ...
    # Load theoretical target returns and gaps
    ...
    # Load 5m data for entry price (lot rounding only)
    ...
    if args.reallocate_unaffordable:
        out_suffix += "_realloc"
    out_dir = ROOT / "results" / f"v2_backtest_lot_rounding_{out_suffix}"
    if args.output_dir:
    ...
    print("\n" + "=" * 60)
    print(f"=== V2 Lot Rounding Backtest (entry={args.entry_mode}, rounding={args.rounding_mode}, realloc={args.reallocate_unaffordable}, capital={initial_capital:,.0f}) ===")
    print("=" * 60)
    ...
    report_lines = [
        "# V2 理論値 vs 整数株・ロット丸めバックテスト\n\n",
        "## 仮定\n\n",
        f"- 執行価格: 5分足 09:10 **{args.entry_mode}**\n",
        f"- 丸め: **{args.rounding_mode}**（'{args.reallocate_unaffordable}' 再配分）\n",
        f"- 初期資本: **{initial_capital:,.0f} JPY**\n",
        "- リターン: 理論値 `compute_jp_target_returns` / `jp_gap` をそのまま使用\n\n",
        "## 期間\n\n",
        ...
        "## 結論\n\n",
        "整数株・ロット丸めを理論値リターンに適用すると、資本が十分大きければ理論値に近づくはず。\n",
        "小資本では高額銘柄・1629.T などが目標ウェイトを実現できず、ポートフォリオが歪む。\n",
    ]
