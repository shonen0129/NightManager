...
def check_pit_history():
    ...
    print(f"\n{'='*70}")
    print("PIT IR History Length Analysis")
    print(f"{'='*70}")
...
def check_omega_struct_stability():
    ...
    print(f"\n{'='*70}")
    print("Omega_struct Stability Across Runs (Step 1)")
    print(f"{'='*70}")
    ...
    for d in dist_dirs:
        ...
        if len(omega_files) > 100:  # Only consider full runs
            dates = [f.stem.split("_")[-1] for f in omega_files if "psd" not in f.stem]
            runs[d.name] = {
                'dir': d,
    ...
    if max_diffs:
        ...
        if max(max_diffs) > 1e-8:
            print("  ** OMEGA_STRUCT MATRICES ARE NOT STABLE ACROSS RUNS **")
            print("  ** This indicates data revision or parameter difference **")
        else:
            print("  ** Omega_struct matrices are stable (no data revision bias) **")
