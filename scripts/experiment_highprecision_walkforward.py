"""High-precision few-signal × high-blend walk-forward OOS test.

Hypothesis: Selecting only top-N highest |IC| pairs allows higher
blend_weight without overfitting.

Tests: top_n = [3, 5, 10, 15, 20] × bw = [0.20, 0.30, 0.40, 0.50, 0.70]
Walk-forward: train=504d, test=63d, step=63d, purge=1d
"""
...
def compute_train_ic_topn(alt_data, y_target, sim_dates, jp_tickers,
    ...
    """Compute IC on train window, return top-N pairs by |IC|."""
    pairs = []
    ...
    # Sort by |IC| descending, take top N
...
def build_combined_for_window(blpx_signals, alt_data, sig_pairs,
    ...
    for sn, tk, ic in sig_pairs:
        ticker_signals.setdefault(tk, []).append((sn, ic))
    for j, tk in enumerate(jp_tickers):
        if tk not in ticker_signals:
            continue
        for sn, ic in ticker_signals[tk]:
            if sn not in alt_data:
                continue
            alt_s = alt_data[sn].reindex(sim_dates).shift(1)
            rm = alt_s.rolling(252, min_periods=60).mean()
            rs = alt_s.rolling(252, min_periods=60).std()
            z = (alt_s - rm) / rs.replace(0, np.nan)
            combined[tk] = combined[tk] + (blend_weight * ic * z).reindex(sim_dates).fillna(0.0).values
    return combined


class WFModel:
    def __init__(self, combined_signals, df_exec, win_start, win_end):
        self.cs = combined_signals
...
def main():
    ...
    ar = float(dr_base.mean() * 245)
    vol = float(dr_base.std(ddof=1) * np.sqrt(245))
    ...
    logger.info("  baseline OOS: Sharpe=%.4f", sharpe)
    ...
    for top_n in top_n_values:
        for bw in blend_weights:
            ...
            for wi, (tr_s, tr_e, te_s, te_e) in enumerate(windows):
                sig_pairs = compute_train_ic_topn(
                    alt_data, y_target, sim_dates, JP_TICKERS, tr_s, tr_e, top_n)
                ...
                if not sig_pairs:
                    ...
                    combined = build_combined_for_window(
                        blpx_signals, alt_data, sig_pairs, sim_dates, JP_TICKERS, bw)
                ...
                if len(dr_w) > 0:
                    all_dr.append(dr_w)
                    all_turnover.append(r["daily_turnover"].reindex(td).dropna())

                if wi % 10 == 0:
                    logger.info("  W%d/%d pairs=%d", wi+1, len(windows), len(sig_pairs))

            dr_all = pd.concat(all_dr) if all_dr else pd.Series(dtype=float)
            ar = float(dr_all.mean() * 245)
            vol = float(dr_all.std(ddof=1) * np.sqrt(245))
            sharpe = ar / vol if vol > 0 else np.nan
            ...
            logger.info("  %s OOS: Sharpe=%.4f MDD=%.2f%% avg_pairs=%.1f", name, sharpe, mdd*100, avg_pairs)

    # Print results
    base_s = all_results[0]["Sharpe"]
    print("\n" + "=" * 120)
    print("HIGH-PRECISION FEW-SIGNAL × HIGH-BLEND WALK-FORWARD OOS")
    print("=" * 120)
    ...
    for r in all_results:
        ...
        print(f"{r['name']:<25} {r['Sharpe']:<10.4f} {ds:<+10.4f} {r['AR']:<10.4f} "
              f"{r['MDD']*100:<8.2f} {r['avg_pairs']:<10.1f}")
    ...
    pd.DataFrame(all_results).to_csv(output_dir / "highprecision_walkforward.csv", index=False)
    print(f"\nSaved to {output_dir}")
