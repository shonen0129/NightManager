...
## 4. シグナルコンポーネント (Raw-PCA 〜 Raw-BLPX)

検証されたシグナルコンポーネントは、アルファソースの数式表現と残差化の組み合わせにより $P$ 番号で分類されている。

```mermaid
graph LR
    Raw-PCA(Raw-PCA: 標準PCA / Raw Target)
    Residual-PCA(Residual-PCA: 標準PCA / TOPIX残差Target)
    P5(P5: Baseline BLP / Raw Target)
    P5P3(P5P3: Baseline BLP / TOPIX残差)
    Raw-BLPX(Raw-BLPX: Enhanced BLP / Raw Target)
    Residual-BLPX(Residual-BLPX: Enhanced BLP / TOPIX残差)

    subgraph PCA_Ensemble_Group["PCA-Ensemble Group"]
        Raw-PCA --> PCA-Ensemble
        Residual-PCA --> PCA-Ensemble
    end

    subgraph BLPX_Ensemble_Group["BLPX-Ensemble Group"]
        Raw-BLPX --> BLPX-Ensemble
        Residual-BLPX --> BLPX-Ensemble
    end

```

### Raw-PCA: Production PCA Signal
* **定義**: 日本ターゲットに **Raw Target** を使用し、標準の PCA Projection を用いて作成されるシグナル。
* **ステータス**: **採用済み (PCA-Ensembleの一部)**。現行PCA-Ensembleモデルの50%のウェイトを占める。

### Residual-PCA: TOPIX Residual PCA Signal
...
* **ステータス**: **不採用（研究継続用）**。実装および監査（Audit）は良好だったが、PCA-Ensembleに対するSharpe改善幅が $+0.02$ 程度と小さく、単体での本番採用には至らなかった。
...
### Raw-BLPX / Residual-BLPX: Enhanced BLP (BLPX)
* **定義**: Baseline BLP を基盤とし、事前情報を用いた縮約（Structured Shrinkage）と不確実性によるシグナル調整を加えた高性能モデル。
  * **Raw-BLPX**: Raw Target を使用する Enhanced BLP。
  * **Residual-BLPX**: TOPIX Residual Target を使用する Enhanced BLP。
...
graph TD
    ...
    F --> Out[シグナル出力: Raw-BLPX / Residual-BLPX]
```
...
これにより、BLPXモデル内に従来のPCA-Ensembleシステムが持つ長期的な関係性の安定性を内包させる。
...
| :--- | :--- | :--- |
| **PCA-Ensemble** | $0.5 \cdot z(Raw-PCA) + 0.5 \cdot z(Residual-PCA)$ | **暫定主力 / フォールバック用**<br>安定した高いARを持つが、本番採用には至っていない。基準ベンチマーク。 |
| **PCA-Ensemble-USR** | $0.4 \cdot Raw-PCA + 0.4 \cdot Residual-PCA + 0.2 \cdot P4$ | **不採用**。米国側市場残差化を追加したが、改善が不十分。 |
| **PCA-Ensemble-USRP** | PCA-Ensemble-USR + Prior 処理 | **不採用**。改善幅が小さく、MDDが悪化する傾向。 |
| **Raw-PCA/P4 Ensemble** | $Raw-PCA$ と $P4$ の複合シグナル | **不採用**。統計的有意性および改善幅が不足。 |
| **PCA-Ensemble-BLP** | $0.5 \cdot z(P5) + 0.5 \cdot z(P5P3)$ | **不採用（研究用）**。実装は極めてクリーンだが、PCA-Ensembleに対する超過リターンがわずか。 |
| **PCA-Ensemble-RRR** | 低ランク回帰系モデル | **不採用**。システム安定性監査に不合格。 |
| **BLPX-Ensemble** | $0.5 \cdot z(Raw-BLPX) + 0.5 \cdot z(Residual-BLPX)$ | **単体採用は非推奨**。ボラティリティは大幅に低減するが、単体ではARがPCA-Ensembleより低下しやすいため、ブレンド素材として扱う。 |
| **PCA-BLPX Hybrid Ensemble (25% Blend)** | $0.75 \cdot z(\text{PCA-Ensemble}) + 0.25 \cdot z(\text{BLPX-Ensemble})$ | **【最有力候補】**<br>PCA-Ensemble的高ARを維持しつつ、BLPXの低リスク・高Sharpe特性を取り込むブレンドモデル。実質的な第一候補。 |
| **PCA-BLPX Hybrid Ensemble** | $0.67 \cdot z(\text{PCA-Ensemble}) + 0.33 \cdot z(\text{BLPX-Ensemble})$ | **【第二候補（アグレッシブ仕様）】**<br>BLPX比率をやや高め、よりシャープレシオの改善を狙う並走用候補。 |
...
| 指標 | Current PCA-Ensemble Baseline | Previous BLPX Best | Best Refined BLPX / Blend<br>(PCA-BLPX Hybrid Ensemble) | 差分 (Blend vs PCA-Ensemble) |
| :--- | :---: | :---: | :---: | :---: |
...
* **5.0 bps (基本基準)**: PCA-Ensemble `3.93` / Refined Blend `4.23`
* **7.5 bps (高スリッページ局面)**: PCA-Ensemble `2.88` / Refined Blend `3.05`
* **10.0 bps (コスト崩壊テスト)**: PCA-Ensemble `1.83` / Refined Blend `1.83`
...
### 9.2 PCA-BLPX Hybrid Ensemble (25% Blend) と PCA-BLPX Hybrid Ensemble の不整合
* **状況**: 推奨比率が25%とされる一方、最良パラメータ（Best Candidate）のテーブル値には33%ブレンドの値が使われていた。
* **解決**: 
  - **最良のパラメータセット** (`rho=0.01`等) においては、`PCA-BLPX Hybrid Ensemble` が最も高い OOS Sharpe (`4.2267`) を達成している。
  - 一方で、`PCA-BLPX Hybrid Ensemble (25% Blend)` もほぼ同等の高いパフォーマンスを維持しており、BLPX-Ensembleのモデルリスクへの依存度を抑えつつPCA-Ensembleの持ち味（高AR）を残すという観点から、**本番初期並走の「第一推奨」としては conservative な 25% ブレンド (`PCA-BLPX Hybrid Ensemble (25% Blend)`)** を採用し、**よりアグレッシブな第二候補として 33% ブレンド (`PCA-BLPX Hybrid Ensemble`)** を位置づける。

### 9.3 Vol-matched scale の値
* **状況**: PCA-Ensembleボラティリティに対してリスクの低い候補をスケールアップする際、スケール係数が 1 未満（0.9822）として出力されていた。
* **解決**: 
  - 最良候補のOOSボラティリティが `19.74%`、目標であるPCA-Ensemble baselineのボラティリティが `22.05%` であるため、数学的なスケール係数は本来 `22.05 / 19.74 = 1.117` となる。
...
### 9.4 PCA-EnsembleとBLPX-Ensembleの相関（0.9024 と 0.41 の混在）
* **状況**: 相関が 0.9024 と極めて高いとする記述と、0.41 と十分に低いとする記述が混在していた。
* **解決**: 
  - **`0.9024`**: **シグナル全体のピアソン相関（Model-level correlation）**。モデル全体としての方向性は一致していることを示す。
  - **`0.41`**: **構成コンポーネント（Raw-PCA, Residual-PCA と Raw-BLPX, Residual-BLPX）自体の相関（Component-level correlation）**。また、ポートフォリオ銘柄選択の重複率（Selection Overlap）は **`48.52%`** である。
