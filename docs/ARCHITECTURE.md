...
results/            # バックテストおよび日次推論の実行出力ルート (PCA-Ensemble本番用)
...
└── leadlag/                 # 戦略パッケージ正本
    ...
    │   └── pca-ensemble.py           # SectorRelativeEnsembleModel 本体
    │
...
| [model_spec_sector_relative_ensemble.md](model_spec_sector_relative_ensemble.md) | PCA-Ensemble モデル仕様（旧本番・フォールバックモデル） |
